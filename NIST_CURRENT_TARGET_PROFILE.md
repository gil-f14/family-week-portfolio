...
| --- | --- | --- | --- | --- | --- | --- |
| Govern | Partial | Private-by-default rules, release gates, claim boundaries, roadmap ownership, documented access decisions, a prioritized risk register, and a vulnerability-management plan with expiring exceptions | A repeatable risk-governance process with approved policies, risk appetite, supplier decisions, review cadence, and operating evidence | Approve risk tolerance; exercise exception expiry and quarterly control review | Product owner / Security reviewer | V2 closeout |
...
| Protect | Partial | Owner-only hosting, least-privilege calendar boundaries, sealed sessions, on-device OCR, bounded input, CSP, review-before-write, protected deletion, seven-day local-draft expiry, and a scoped two-step local-data reset | Tested preventive controls for identity lifecycle, shared-device privacy, retention, recovery, and supported endpoints | Test expiry and reset on supported devices; test disconnect/revocation; independently review access control | Engineering owner / Privacy reviewer | V2 closeout |
...
| Respond | Partial | Private vulnerability reporting, containment and credential-rotation guidance, rollback rules, stop-work conditions, and a documented synthetic decision tabletop | A tested incident process covering triage, containment, evidence protection, notification decisions, and lessons learned | Run an authorized operational drill; maintain restricted contacts and obtain notification-process review | Incident owner / Product owner | V2 operations |
| Recover | Partial | Known-good private deployment, version rollback path, retry-safe identifiers, duplicate checks, cleanup procedures, and documented synthetic recovery gates | Tested restoration with recovery objectives, verified configuration, post-incident validation, and stakeholder communication | Define recovery-time and recovery-point objectives; rehearse actual rollback and configuration validation | Operations owner | Before broader access |
...
| Govern-P | Partial | Data-minimization rules, public-release boundary, standards claim policy, human approval, access restrictions, and a current-versus-target retention schedule | Approved privacy roles, policies, risk tolerance, retention schedule, access reviews, and supplier criteria | Approve retention decisions; assign review cadence; document exception handling and provider reassessment | Product owner / Privacy reviewer | V2 operations |
...
| Communicate-P | Partial | In-product local-processing messages, visible uncertainty, portfolio claim boundaries, security reporting instructions, and a concise privacy notice draft covering processing, storage, sharing, choices, retention, and support | An independently approved user-facing notice kept accurate as processing changes | Independently review and approve the draft before adding shared users | Product owner / Privacy reviewer | Before broader access |
| Protect-P | Partial | Same-origin OCR assets, input bounds, secure session cookie, owner-only hosting, CSP, no-sensitive-logging rule, an automated runtime source boundary against unapproved logging/telemetry paths, and restricted calendar writes | Independently tested confidentiality, integrity, identity lifecycle, endpoint, provider logging, and disposal controls | Complete manual device tests, inspect provider-default logs, penetration testing, and revocation exercises | Security reviewer / Engineering owner | Before broader access |
...
| P1 | Incident decisions have a synthetic tabletop, but operational containment and recovery are not exercised | Procedures may fail under real permissions or provider constraints | Run an authorized non-production containment, rotation, alert, and rollback drill | Timed sanitized record and closed corrective actions |
