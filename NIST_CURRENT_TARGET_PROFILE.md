...
| Protect | Partial | Owner-only hosting, least-privilege calendar boundaries, sealed sessions, on-device OCR, bounded input, CSP, review-before-write, and protected deletion | Tested preventive controls for identity lifecycle, shared-device privacy, retention, recovery, and supported endpoints | Test disconnect/revocation; define local-data reset and retention behavior; independently review access control | Engineering owner / Privacy reviewer | V2 closeout |
| Detect | Partial | GitHub secret, dependency, and push-protection monitoring; build integrity checks; visible application failure states | Privacy-safe detection across authentication failures, calendar API errors, security control changes, and deployment health | Define content-free security events, alert thresholds, owners, and retention; test alert routing | Operations owner | V2 operations |
...
| Function | Current posture | Current evidence | Target outcome | Priority evidence or action | Accountable role | Target phase |
| --- | --- | --- | --- | --- | --- | --- |
| Identify-P | Implemented for current design scope | Data-flow map, sensitivity classification, local-versus-server boundaries, external-service inventory, and misuse cases | A maintained record of processing purposes, data elements, locations, recipients, retention, and risks to individuals | Complete a privacy-impact assessment and data-retention inventory using synthetic descriptions | Privacy reviewer / Product owner | V2 closeout |
...
| Control-P | Partial | User reviews OCR and event details, chooses readable calendar overlays, controls calendar mutations, and can clear device-local drafts | Clear controls for viewing, correcting, deleting, exporting, and resetting device-local data without hidden synchronization | Add a consolidated local-data review/reset surface and test it on supported devices | Product owner / Design owner | V3 discovery |
...
| P1 | Device-local retention and reset controls are fragmented | Shared or lost device may expose drafts or learned corrections | Define retention; add consolidated review and reset control | Verified reset test and updated privacy notice |
| P1 | Privacy-safe operational detection is not defined | Security or reliability failures may be missed | Specify content-free events, thresholds, ownership, and retention | Alert test without calendar or household content |
