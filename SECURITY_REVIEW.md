...
| Threat-model coverage | Baseline prioritized; independent review pending | Ten synthetic risk scenarios identify trust boundaries, residual priority, current treatment, and required closeout evidence, including OCR benchmark substitution, result tampering, threshold changes, and baseline poisoning; no high-priority risk is represented as accepted or closed |
...
| Supply-chain evidence linkage | Implemented; not independently validated | A deterministic private SHA-256 record links the manifest, lockfile, SBOM, inventory, review worksheet, artifact trace, and candidate notice. An automated consistency gate reconciles all SBOM and inventory package/version/license rows, the 11-item review queue, open distribution status, required evidence paths, notice references, and privacy patterns; this is freshness and consistency evidence, not a signature, legal decision, or provenance attestation |
| Private build-artifact safety | Passed for current build | The release gate rejects source maps, unclassified file types, credential-like values, emails, local paths, OAuth client identifiers, and unexpected private deployment metadata; the expected hosting manifest remains explicitly private-only and non-distributable |
| Automated release preflight | Passed | Lint, production build, all 119 automated tests, public-readiness claim-boundary synchronization, accessibility manual-evidence boundary protection, pending-state protection for the ten-case calendar test plan, runtime logging/telemetry source boundary, candidate third-party notice integrity, OCR integrity, synthetic benchmark-template freshness, fail-closed benchmark scoring and regression enforcement, benchmark-integrity threat coverage, benchmark-record safety, build-artifact safety, security-inventory consistency and evidence-linkage freshness, portfolio privacy patterns, local links, and unexpected-file controls passed |
| Runtime logging boundary | Source guarded; provider verification pending | Application, library, and worker sources contain no console logging, generic logger calls, browser beacon delivery, or imports from common telemetry SDKs; the prior client OCR exception is not logged. Hosting, identity, edge, browser, and provider-default logs remain outside this check and require inspection |
...
| Vulnerability management | Synthetic decision path exercised; operations not validated | Private intake, severity targets, remediation and verification stages, evidence minimization, time-bounded exception rules, and two synthetic scenarios are documented; live operations and independent review remain pending |
| Privacy-safe monitoring | Validator and local thresholds tested; instrumentation inactive | A pure fail-closed module automates the schema and threshold cases without storage or network side effects; retention jobs, access enforcement, collection, alert delivery, operating exercises, and independent review remain pending |
| OCR evaluation | Synthetic benchmark designed; template, scorer, comparator, and blank execution record tested; execution pending | A privacy-safe forty-image minimum matrix defines two-reviewer ground truth, character/word/field measures, zero unsafe-pass tolerance, accuracy thresholds, regression criteria, and evidence limits. A deterministic private forty-case template is bound to the four pinned OCR asset hashes. Fail-closed validation, scoring, and comparison reject stale or mismatched evidence, free-form fields, incomplete results, invalid counts, unblocked ambiguity, threshold failures, and regressions over two percentage points. A public-safe record constrains execution, metric, failure, and approval evidence; image creation, representative execution, approved baseline selection, and independent review remain pending |
| Accessibility conformance | Not yet claimed | Automated and source-level evidence passed, including a zero-violation axe-core 4.13.0 audit of the unauthenticated root state and a synthetic authenticated-state contract covering dialog labels, focus containment/return, responsive chooser bounds, and polite view-state announcements. Keyboard bypass navigation is available, a low-contrast timeline label was remediated to a 5.15:1 contrast ratio, and decorative upload movement respects reduced-motion preferences. Authenticated runtime, manual assistive-technology, and responsive-state testing remains |
